...
test("Path 1", () => {
  ...
  expect(p.path).toBe("/this/is/a/path");
});
...
test("Prefix 1", () => {
  ...
  expect(p.parent?.name).toBe("a");
  expect(p.path).toBe("/this/is/a/path");
  expect(p.removePrefix("/this/").path).toBe("is/a/path");
});
...
test("Prefix 4", () => {
  const p = new Path("/this/is/a/path/");
  expect(p.removePrefix("/this/is").path).toBe("a/path");
});
...
test("Concat 1", () => {
  ...
  expect(p3.path).toBe("/this/is/a/path/foo");
})
